...
class Hidden:
    ...
    def train_on_batch(self, batch: list):
        ...
        with torch.enable_grad():
            ...
            # train on cover
            #创建形状为(batch_size, 1)的张量，用于存储真实标签
            d_target_label_cover = torch.full((batch_size, 1), self.cover_label, device=self.device)
            #创建形状为(batch_size, 1)的张量，用于存储加密图像的标签
            d_target_label_encoded = torch.full((batch_size, 1), self.encoded_label, device=self.device)
            #创建形状为(batch_size, 1)的张量，用于存储生成器目标标签
            g_target_label_encoded = torch.full((batch_size, 1), self.cover_label, device=self.device)
            #前向传播,用判别器判别输入cover_image,计算损失，最后反向传播
            d_on_cover = self.discriminator(images)
            d_loss_on_cover = self.bce_with_logits_loss(d_on_cover, d_target_label_cover.float())
            d_loss_on_cover.backward()

            # train on fake
            #前向传播,用判别器判别生成的encoded_image,计算损失，最后反向传播
            #encoded_images: 编码器输出的加密图像
            #noised_images: 编码器输出的噪声图像
            #decoded_messages: 解码器输出的恢复消息
        ...
        #将解密出的信息先停止梯度追踪，避免反向传播时计算梯度，转移到CPU并转换为numpy数组，最后四舍五入并裁剪到0-1之间
        decoded_rounded = decoded_messages.detach().cpu().numpy().round().clip(0, 1)
        #计算位错误率，解密出的信息减去真实消息，取绝对值，再求和，最后除以总像素数，得到位错误率
        bitwise_avg_err = np.sum(np.abs(decoded_rounded - messages.detach().cpu().numpy())) / (
                batch_size * messages.shape[1])

        losses = {
            'loss           ': g_loss.item(), #生成器总损失
            'encoder_mse    ': g_loss_enc.item(),#编码器损失，用于让编码器输出与真实图像的MSE最小化
            'dec_mse        ': g_loss_dec.item(),#解码器损失，用于让解码器输出与真实消息的MSE最小化
            'bitwise-error  ': bitwise_avg_err,#描述生成器的损失，用于评估生成器的恢复消息与真实消息的位错误率
            'adversarial_bce': g_loss_adv.item(),#生成器损失，用于让生成器输出的加密图像被判别器识别为真实图像
            'discr_cover_bce': d_loss_on_cover.item(),#判别器损失，用于让判别器输出与真实图像的BCE最小化
            'discr_encod_bce': d_loss_on_encoded.item()#判别器损失，用于让判别器输出与加密图像的BCE最小化
        }
